            ...
            if (camera_children)
            {
                if (camera_child_count > 0)
                {
                    ...
                    if (children)
                    {
                        for (long i = 0 ; i < child_count ; i++)
                        {
                            vm_address_t part = children[i].child_address;
                            if (rbx_instance_is_a(task, part, "Part"))
                            {
                                if (rbx_instance_find_first_child_of_class(task, part, "Motor6D"))
                                {
                                    trigger = part;
                                    i = child_count;
                                }
        ...
        for (;;)
        {
            ...
            if (enemies_list)
            {
                ...
                for (int i = 0 ; i < enemy_count ; i++)
                {
                    ...
                    if (enemy)
                    {
                        vm_address_t torso = pf_get_character_part_by_volume(task, enemy, target_character_part_volume);
...
#pragma mark - ESP -
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
    {
        for (;;)
        {
            ...
            COPY_VAR(current_enemy_count)
            COPY_BUF(enemy_torsos)
            if (esp_enabled &&
                ...
                __current_enemy_count < (max_player_count + 1))
            {
                ...
                for (long i = 0 ; i < __current_enemy_count ; i++)
                {
                    ...
                    vm_address_t enemy_torso = __enemy_torsos[i];
                    if (enemy_torso)
                    {
                        ...
                        if (rbx_instance_is_valid_child(task, enemy_torso))
                        {
                            //printf("DRAWING ESP\n");
            ...
            if (aimbot_enabled)
            {
                    ...
                    camera_child_count > 2)
                {
                    if (shared_memory->LEFT_MOUSE_DOWN == true)
                    {
                        ...
                        vector3_t new_look_vector = vector3_direction_to_from(torso_cframe.pos, trigger_cframe.pos);
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
            ...
            for (;;)
            {
                ...
                if (rd.didRecognize == true)
                {
                    ...
                    {
                        jarvis_talk("enabling E-S-P");
                        esp_enabled = true;
                        
                    }
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[1]])
                    {
                        jarvis_talk("disabling E-S-P");
                        esp_enabled = false;
                        
                    }
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[2]])
                    {
                        jarvis_talk("enabling aim bot");
                        aimbot_enabled = true;
                        
                    }
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[3]])
                    {
                        jarvis_talk("disabling aim bot");
                        aimbot_enabled = false;
                        
                    }
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[4]])
                    {
                        jarvis_talk("switching target team");
                        __t__0 = abs(--__t__0);
                        __t__1 = abs(--__t__1);
                    }
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[5]])
                    {
                        max_delta_dist = 3;
                        jarvis_talk("setting aimbot size to 3");
                    }
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[6]])
                    {
                        max_delta_dist = 5;
                        jarvis_talk("setting aimbot size to 5");
                    }
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[7]])
                    {
                        max_delta_dist = 7;
                        jarvis_talk("setting aimbot size to 7");
                    }
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[8]])
                    {
                        max_delta_dist = 9;
                        jarvis_talk("setting aimbot size to 9");
                    }
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[9]])
                    {
                        inacc =  0;
                        jarvis_talk("setting inaccuracy to 0");
                    }
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[10]])
                    {
                        inacc =  2;
                        jarvis_talk("setting inaccuracy to 2");
                    }
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[11]])
                    {
                        inacc = 4;
                        jarvis_talk("setting inaccuracy to 4");
                    }
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[12]])
                    {
                        inacc = 6;
                        jarvis_talk("setting inaccuracy to 6");
                    }
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[13]])
                    {
                        target_character_part_volume = 1;
                        jarvis_talk("targetting heads");
                    }
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[14]])
                    {
                        target_character_part_volume = 4;
                        jarvis_talk("targetting torsos");
                    }
                    rd.spokenCommand = @"";
                }
            }
