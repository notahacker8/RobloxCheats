...
void phantom_forces_cheat(task_t task, float __aimbot_max_delta_dist)
{
    ...
    static float max_delta_dist = 0;
    max_delta_dist = __aimbot_max_delta_dist;
...
#pragma mark - ESP -
    ...
    if (esp_enabled)
    {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
            ...
            for (;;)
            {
                if (current_enemy_count > 0 && current_enemy_count < (RBX_PHANTOM_FORCES_MAX_PLAYER_COUNT + 1))
                {
                    int esp_index = 0;
