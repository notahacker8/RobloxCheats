...
vm_address_t pf_get_character_part_by_volume(task_t task, vm_address_t task_base_address, vm_address_t character, float volume)
{
        ...
        for (;;)
        {
            ...
            if (camera_children)
            {
                if (camera_child_count > 0)
                {
                    ...
                    camera_part = rbx_instance_find_first_child_by_pointee(task, camera, task_base_address, RBX_OBJECT_Part_POINTEE_OFFSET);
                    ...
                    if (children)
                    {
                        for (long i = 0 ; i < child_count ; i++)
                        {
                            ...
                            if (vm_read_uint64_t_value(task, obj) == task_base_address + RBX_OBJECT_Part_POINTEE_OFFSET)
                            {
                                //rbx_print_descendants(task, obj, 0, 0);
                                vm_address_t motor6d = rbx_instance_find_first_child_by_pointee(task, obj, task_base_address, RBX_OBJECT_Motor6D_POINTEE_OFFSET);
                                if (motor6d)
                                {
                                   /* for (int x = 0 ; x < 100 ; x++)
                                    {
                ...
                for (int i = 0 ; i < enemy_count ; i++)
                {
                    ...
                    if (enemy)
                    {
                        ...
                        FOR_EACH_DESCENDANT(task, enemy, 1000, 4, i, {
                            ...
                            if (vm_read_uint64_t_value(task, obj) == task_base_address + RBX_OBJECT_TextLabel_POINTEE_OFFSET)
                            {
                                long enemy_name_l = 0;
