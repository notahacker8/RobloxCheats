...
#pragma mark - Object Loops -
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        for (;;)
        {
            ...
            my_hrp = vm_read_uint64_t_value(task, my_character + RBX_MODEL_PRIMARYPART_OFFSET);
            ...
            if (enemies_list)
            {
                ...
                for (int i = 0 ; i < enemy_count ; i++)
                {
                    ...
                    if (enemy)
                    {
                        ...
                        vm_address_t descendants[1000];
                        rbx_get_descendants(task, enemy, 0, 4, descendants, &descendant_count, sizeof(descendants)/8);
                        for (int x = 0 ; x < descendant_count ; x++)
                        {
                            vm_address_t d = descendants[x];
                            if (rbx_instance_is_a(task, d, "TextLabel"))
                            {
                                enemy_name = rbx_textlabel_get_text(task, d, &enemy_name_l);
                            }
                        }
                        ...
                        if (torso && parent && instance_self == torso)
                        {
                            ...
                            APPEND_ESP_OBJ(torso, esp_color, enemy_name)
                            if (enemy_name)
                            {
