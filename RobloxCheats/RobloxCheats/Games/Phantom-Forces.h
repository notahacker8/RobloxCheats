...
void phantom_forces_cheat(task_t task)
{
    ...
    static float max_delta_dist = 20;
                        ...
                        for (long i = 0 ; i < child_count ; i++)
                        {
                            ...
                            if (rbx_instance_is_a(task, obj, "Part") == true)
                            {
                                ...
                                if (motor6d)
                                {
                                   /* for (int x = 0 ; x < 100 ; x++)
                                    {
                                        vm_address_t v = vm_read_uint64_value(task, motor6d + (x * 8));
            ...
            if (aimbot_enabled)
            {
                    ...
                    camera_child_count > 2)
                {
                    ...
                    //if(true)
                    {
                        ...
                        vector3_t new_cam_part_look_vector = vector3_direction_to_from(torso_cframe.pos, camera_cframe.pos);
                        
                        //new_cam_part_look_vector.y = 0;
                        
                        //new_cam_part_look_vector = vector3_normalized(new_cam_part_look_vector);
