...
/*
@TODO: Check "if (vm_read_uint32_t_value(task, stats_frame + 0x2b0) != 0x3f000000)" using BitSlicer
 */
...
void blox_fruits_auto_farm(task_t task, bfa_info_t __bfa_info)
{
    ...
    printf("\tCheck \"if (vm_read_uint32_t_value(task, stats_frame + 0x2b0) != 0x3f000000)\" using BitSlicer\n");
    pid_t pid = -1;
    ...
    if (bfa_info.hunt_sea_events)
    {
        bfa_info.is_partial_name = true;
        bfa_info.enemy_name = "2000";
    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        for (;;)
        {
            ...
            else
            {
                if (bfa_info.hunt_sea_events)
                {
                    vm_address_t enemy_boat = blox_fruits_find_first_alive_ship(task, enemies_folder);
                    if (enemy_boat)
                    {
                        target_enemy = enemy_boat;
                    }
                    else
                    {
...
#pragma mark stats
            ...
            get_stats: {}
                ...
                if (stat_points_available > 0)
                {
                    ...
                    if (vm_read_uint32_t_value(task, stats_frame + 0x2b0) != 0x3f000000)
                    {
                ...
                if (stat_points_available < 1)
                {
                    ...
                    SEND_MOUSEMOVE_YIELDING(0, 700, 300);
                }
...
#pragma mark attacking
            ...
            if (race_transformed)
            {
                ...
                {
                    if (bfa_info.lhk_tool_key == bfa_info.racev4_tool_key)
                    {
            ...
            else
            {
                ...
                if (bfa_info.lhk_tool_key == bfa_info.tool_key)
                {
                    switch_weapons = false;
                }
            ...
            if (do_last_hit_kill_check)
            {
                if (__target_enemy_hum)
                {
                    ...
                    if (reserve_health >= health && health > 0)
                    {
                        ...
                        {
                            SEND_KEYPRESS_YIELDING(2, 'z', 500000);
                            SEND_KEYPRESS_YIELDING(2, 'x', 500000);
                            SEND_KEYPRESS_YIELDING(2, 'c', 500000);
                        }
                        SEND_KEYPRESS_YIELDING(2, tool_key, 10000);
                        rbx_camera_set_camera_subject(task, camera, my_character);
                    }
            ...
            {
                SEND_LEFTCLICK_YIELDING(1, 100000)
            }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        for (;;)
        {
            ...
            if (rbx_instance_is_valid_child(task, __my_hrp) && (rbx_humanoid_get_health(task, __my_hum) > 0.0) && !rbx_humanoid_get_seatpart(task, my_hum))
            {
                ...
                {
                    if (quest_giver_part || target_enemy_hrp || (bfa_info.hunt_sea_events && my_boat_seat))
                    {
                        should_write_cframe = true;
                    }
                    else
...
#pragma mark don't do quest
                else
                {
                    ...
                    else
                    {
                        if (bfa_info.hunt_sea_events)
                        {
                            //printf("no enemies, going to boat seat\n");
                            //rbx_print_descendants(task, my_boat_seat, 0, 0);
                            __dest_pos = rbx_basepart_get_cframe(task, my_boat_seat).pos;
                        }
                        if (bfa_info.use_proxy_positons)
                        {
                            __dest_pos = bfa_info.enemy_area_proxy_pos;
