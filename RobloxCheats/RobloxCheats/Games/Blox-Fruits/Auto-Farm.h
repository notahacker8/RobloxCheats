                           ...
                           char use_f)
{
    ...
    static float max_teleport_range = 125;
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        for (;;)
        {
            ...
            target_enemy = blox_fruits_find_first_alive_enemy_by_name(task, enemies_folder, enemy_name);
            ...
            if (!tool)
            {
                shared_memory->INPUT_QUEUE[6] = (Input){.type = 1, .should_execute = true, .keycode = keycode_for_character(tool_key)};
                shared_memory->INPUT_QUEUE_FINISHED = false;
                while (shared_memory->INPUT_QUEUE_FINISHED == false) {usleep(10000);}
                shared_memory->INPUT_QUEUE[6] = (Input){.type = 2, .should_execute = true, .keycode = keycode_for_character(tool_key)};
                shared_memory->INPUT_QUEUE_FINISHED = false;
                while (shared_memory->INPUT_QUEUE_FINISHED == false) {usleep(10000);}
            }
            while (shared_memory->INPUT_QUEUE_FINISHED == false) {usleep(10000);}
            shared_memory->INPUT_QUEUE[0] = (Input){.type = 3, .should_execute = true, .x = shared_memory->MOUSE_X, .y = shared_memory->MOUSE_Y};
            if (use_z) { shared_memory->INPUT_QUEUE[1] = (Input){.type = 1, .should_execute = true, .keycode = keycode_for_character('z')}; }
            if (use_x) { shared_memory->INPUT_QUEUE[2] = (Input){.type = 1, .should_execute = true, .keycode = keycode_for_character('x')}; }
            if (use_c) { shared_memory->INPUT_QUEUE[3] = (Input){.type = 1, .should_execute = true, .keycode = keycode_for_character('c')}; }
            if (use_v) { shared_memory->INPUT_QUEUE[4] = (Input){.type = 1, .should_execute = true, .keycode = keycode_for_character('v')}; }
            if (use_f) { shared_memory->INPUT_QUEUE[5] = (Input){.type = 1, .should_execute = true, .keycode = keycode_for_character('f')}; }
            shared_memory->INPUT_QUEUE_FINISHED = false;
            ...
            shared_memory->INPUT_QUEUE[0] = (Input){.type = 4, .should_execute = true, .x = shared_memory->MOUSE_X, .y = shared_memory->MOUSE_Y};
            if (use_z) { shared_memory->INPUT_QUEUE[1] = (Input){.type = 2, .should_execute = true, .keycode = keycode_for_character('z')}; }
            if (use_x) { shared_memory->INPUT_QUEUE[2] = (Input){.type = 2, .should_execute = true, .keycode = keycode_for_character('x')}; }
            if (use_c) { shared_memory->INPUT_QUEUE[3] = (Input){.type = 2, .should_execute = true, .keycode = keycode_for_character('c')}; }
            if (use_v) { shared_memory->INPUT_QUEUE[4] = (Input){.type = 2, .should_execute = true, .keycode = keycode_for_character('v')}; }
            if (use_f) { shared_memory->INPUT_QUEUE[5] = (Input){.type = 2, .should_execute = true, .keycode = keycode_for_character('f')}; }
            shared_memory->INPUT_QUEUE_FINISHED = false;
            while (shared_memory->INPUT_QUEUE_FINISHED == false) {usleep(10000);}
        ...
        for (;;)
        {
            ...
            if (should_do_quest)
            {
                ...
                else
                {
                    if (quest_giver_part)
                    {
                        ...
                        vector3_t __new_pos = rbx_basepart_get_cframe(task, quest_giver_part).pos;
                        if (vector3_dist_dif(__new_pos, my_hrp_cframe.pos) > max_teleport_range)
                        {
            ...
            else
            {
                if (target_enemy_hrp)
                {
                    ...
                    {
                        rbx_cframe_t target_hrp_cframe = rbx_basepart_get_cframe(task, target_enemy_hrp);
                        vector3_t __new_pos = vector3_add(rbx_basepart_get_cframe(task, target_enemy_hrp).pos, (vector3_t){.x = 0, .y = y_offset,.z = 0});
                        if (vector3_dist_dif(__new_pos, my_hrp_cframe.pos) > max_teleport_range)
                        {
                            __new_pos = vector3_add(my_hrp_cframe.pos, vector3_mul_num(vector3_unit(target_hrp_cframe.pos, my_hrp_cframe.pos), max_teleport_range));
                            is_too_far = true;
