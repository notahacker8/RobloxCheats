                                                        ...
                                                        char is_partial_name)
{
    ...
    FOR_EACH_CHILD(enemies_folder, i, {
        ...
        vm_address_t humanoid = rbx_instance_find_first_child_by_pointee(task, enemies_folder, task_base_address, RBX_OBJECT_Humanoid_POINTEE_OFFSET);
        if (humanoid)
        {
...
void bf_equip_until_weapon_name_shown(task_t task, vm_address_t tl, vm_address_t character, char* wn, libESPSharedMemoryObject* shared_memory)
{
    char key = '0';
    for (char i = 0 ; i < 9 ; i++)
    {
        if (rbx_textlabel_equals_text(task, tl, wn) && rbx_instance_find_first_child_of_class(task, character, "Tool"))
        {
            return;
        }
        else
...
#pragma mark attacking
            ...
            if (bfa_info.lhk_default_enabled || (bfa_info.lhk_v4_enabled && race_transformed))
            {
                if (__target_enemy_hum)
                {
                    ...
                    if (reserve_health >= health && health > 0)
                    {
                        ...
                        {
                            bf_equip_until_weapon_name_shown(task, skills_frame_title_tl, my_character, bfa_info.lhk_weapon_name, shared_memory);
                        }
                        else
                        {
                        ...
                        {
                            //KEYPRESSES.
                            send_keypress_yielding(shared_memory, 2, keycode_for_character('z'), "z", 100000);
                            send_keypress_yielding(shared_memory, 2, keycode_for_character('x'), "x", 100000);
                            send_keypress_yielding(shared_memory, 2, keycode_for_character('f'), "f", 100000);
                        }
                        rbx_camera_set_camera_subject(task, camera, my_character);
                        if (bfa_info.use_weapon_names)
                        {
            ...
            {
                bf_equip_until_weapon_name_shown(task, skills_frame_title_tl, my_character, name_of_weapon_to_equip, shared_memory);
            }
