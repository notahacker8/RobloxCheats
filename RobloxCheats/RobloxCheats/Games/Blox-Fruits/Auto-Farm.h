...
#pragma mark attacking
            ...
            if (bfa_info.auto_race_transform)
            {
                if (truncl(race_energy) == 1)
                {
                    ...
                    if (bfa_info.auto_tool_equip)
                    {
                        ...
                        if (tool_key != bfa_info.tool_key)
                        {
                            send_keypress_yielding(shared_memory, 2, keycode_for_character(tool_key), &tool_key, 100000);
                        }
                        //transformed = true;
                    }
                }
            ...
            char switch_weapons = false;
            char lhk_switch_weapons = false;
            
            useconds_t click_dur = bfa_info.click_duration;
            if (race_transformed)
            {
                tool_key = bfa_info.racev4_tool_key;
                click_dur = bfa_info.v4_click_duration;
                if (bfa_info.lhk_v4_enabled)
                {
                    if (bfa_info.lhk_tool_key == bfa_info.racev4_tool_key)
                    {
                        lhk_switch_weapons = false;
                    }
            ...
            {
                send_leftclick_yielding(shared_memory, 1, shared_memory->MOUSE_X, shared_memory->MOUSE_Y, click_dur);
            }
