...
{
    static char jarvis_enabled = false;
...
#pragma mark - Globals -
    ...
    shared_memory->FUNCTION_COUNT = 6;
...
#pragma mark - Object Loops -
    ...
    /*dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
    {
        for (;;)
        {
            ...
            vector3_t add = {.x = 0, .y = 2, .x = 0};
            rbx_cframe_t new_cf = my_hrp_cframe;
            new_cf.pos = vector3_add(new_cf.pos, add);
            new_cf.pos = vector3_add(new_cf.pos, vector3_mul_num(lv, 6));
...
#pragma mark - NoClip -
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        for (;;)
        {
            if (noclip_enabled || item_collect_enabled || levi_bot_should_follow_seat || auto_raid_enabled)
            {
...
#pragma mark - Fly -
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
    {
        for (;;)
        {
            ...
            {
                if (auto_raid_enabled && next_raid_island_part)
                {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
    {
        for (;;)
        {
            if (rbx_humanoid_get_seatpart(task, my_humanoid) && !is_levi_bot_program)
            {
                vector3_t add = {};
...
#pragma mark - stdin Inputs -
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        for (;;)
        {
            ...
            if (strcmp(str, "jarvis\xa") == 0)
            {
                printf("ENABLING JARVIS...\n");
                jarvis_enabled = true;
            }
    ...
    @autoreleasepool {
        RecognizerDelegate* rd = [[RecognizerDelegate alloc] init];
        rd.recognizer.commands = @[@"Show Fruit", //0
                                   @"Expand Hit Boxes", //1
                                   @"No Hit Boxes", //2
                                   @"Embers", //3
                                   @"Mirage Chests", //4
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
            ...
            {
                usleep(100000);
                if (rd.didRecognize == true)
                {
                    rd.didRecognize = false;
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[0]])
                    {
                        jarvis_talk("showing most recent fruit");
                        vm_address_t s = rbx_instance_find_first_child(task, workspace, "Fruit ");
                        if (s)
                        {
                            rbx_camera_set_camera_subject(task, camera, s);
                    ...
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[1]] == true)
                    {
                        new_npc_hitbox_size = (vector3_t){.x = 120, .y = 120, .z = 120};
                        jarvis_talk("expanding NPC hitboxes");
                    };
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[2]] == true)
                    {
                        new_npc_hitbox_size = (vector3_t){.x = 12, .y = 12, .z = 12};
                        jarvis_talk("minimizing NPC hitboxes");
                    };
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[3]] == true)
                    {
                        rbx_smooth_fly(task, 1);
                        fly_enabled = true;
                        item_collect_enabled = true;
                        item_collection_mode = BLOX_FRUITS_ITEM_COLLECTION_MODE_EMBERS_ONLY;
                        jarvis_talk("collecting only embers");
                    };
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[4]] == true)
                    {
                        rbx_smooth_fly(task, 1);
                        fly_enabled = true;
                        item_collect_enabled = true;
                        item_collection_mode = BLOX_FRUITS_ITEM_COLLECTION_MODE_MIRAGE_CHESTS_ONLY;
                        jarvis_talk("collecting only mirage island chests");
                    };
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[5]] == true)
                    {
                        rbx_smooth_fly(task, 1);
                        fly_enabled = true;
                        item_collect_enabled = true;
                        item_collection_mode = BLOX_FRUITS_ITEM_COLLECTION_MODE_CHESTS_ONLY;
                        jarvis_talk("collecting only chests");
                    };
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[6]] == true)
                    {
                        noclip_enabled = true;
                        jarvis_talk("enabling no clip");
                    };
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[7]] == true)
                    {
                        noclip_enabled = false;
                        jarvis_talk("disabling no clip");
                    };
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[8]] == true)
                    {
                        aimbot_enabled = true;
                        jarvis_talk("enabling aim bot");
                    };
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[9]] == true)
                    {
                        aimbot_enabled = false;
                        jarvis_talk("disabling aim bot");
                    };
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[10]] == true)
                    {
                        fly_speed = 150;
                        jarvis_talk("set fly speed to 150");
                    };
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[11]] == true)
                    {
                        fly_speed = 275;
                        jarvis_talk("set fly speed to 275");
                    };
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[12]] == true)
                    {
                        raid_assist_enabled = true;
                        jarvis_talk("enabled raid assist");
                    };
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[13]] == true)
                    {
                        raid_assist_enabled = false;
                        jarvis_talk("disabled raid assist");
                    };
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[14]] == true)
                    {
                        auto_raid_enabled = true;
                        raid_assist_enabled = true;
                        fly_enabled = true;
                        rbx_smooth_fly(task, fly_enabled);
                        jarvis_talk("enabled auto raid");
                    };
                    if ([rd.spokenCommand isEqualToString:rd.recognizer.commands[15]] == true)
                    {
                        auto_raid_enabled = false;
                        raid_assist_enabled = false;
                        fly_enabled = false;
                        rbx_smooth_fly(task, fly_enabled);
                        jarvis_talk("disabled auto raid");
                    };
                    rd.spokenCommand = @"";
                }
            }
