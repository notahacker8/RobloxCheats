...
void blox_fruits_cheat(task_t task)
{
    ...
    ///vm_address_t basepart_set_velocity_func = task_base_address + RBX_BASEPART_ASSEMBLYLINEARVELOCITY_SETTER_METHOD_OFFSET;
    ...
    //03 68 F6 3C 03 3C 80 3D 02 38 80 3D (intel)
#ifdef ROBLOX_ROSETTA_VERSION
    static char fly_bytes[12] = {"\x03\x68\xF6\x3C\x03\x3C\x80\x3D\x02\x38\x80\x3D"};
#endif
#ifdef ROBLOX_APPLE_VERSION
    static char fly_bytes[12] = {"\x20\x0D\xC0\x3D\x00\x3D\x80\x3D\x2A\x21\x40\xF9"};
#endif
    static char orig_bytes[4];
    memcpy(orig_bytes, fly_bytes + 4, 4);
    static char NOP[4] = {"\x1f\x20\x03\xd5"};
    ...
    vm_address_t sky = rbx_instance_find_first_child(task, lighting_service, "Sky");
    ...
    static Input mouse_aimbot_input = {.type = 0};
        ...
        for (;;)
        {
            ...
            if (children)
            {
                for (long i = 0 ; i < child_count ; i++)
                {
                    ...
                    {
                        //rbx_print_descendants(task, object, 0, 2);
