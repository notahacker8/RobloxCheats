            ...
            if (lock_onto_npc)
            {
                if (my_hrp && (rbx_humanoid_get_health(task, my_humanoid) > 0))
                {
                    ...
                    FOR_EACH_CHILD(enemies_folder, i, {
                        ...
                        if ((hum && (rbx_humanoid_get_health(task, hum) > 0)) || child == dungeon_boss)
                        {
            ...
            FOR_EACH_CHILD(players_service, i, {
                ...
                if (player != local_player)
                {
                    ...
                    if (name)
                    {
                        ...
                        {
                            //vm_address_t fmp = rbx_instance_find_first_child(task, c, "Head");
                            
                            //rbx_print_descendants(task, fmp, 0, 0);
                            //float sz = 30;
                            //rbx_basepart_set_size(task, h, (vector3_t){.x = sz,. y = sz, .z = sz});
                            //blox_fruits_expand_hitbox(task, player, og_hitbox_size);
...
#pragma mark Instances [6]
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        for (;;)
        {
            ...
            //rbx_print_descendants(task, my_tool, 0, 0);
            if (rbx_instance_is_named(task, my_tool, "Dragonstorm"))
            {
                ...
                if (!dungeon_mode_enabled)
                {
                    ...
                    {
                        rbx_basepart_set_size(task, other_player_hrps[i], new_plr_hitbox_size);
                    }
                }
...
#pragma mark n
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        {
            usleep(1000);
            if (boat_keybind_enabled && boathacks_enabled)
            {
                if (shared_memory->KEYS_DOWN['='] == true)
                {
                    if (my_humanoid_current_seat_part)
                    {
                        high_boat = !high_boat;
                        while (shared_memory->KEYS_DOWN['='] == true)
                        {
                            usleep(500);
                        }
                    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        for (;;)
        {
            if (lock_onto_npc)
            {
                ...
                {
                    if (rbx_humanoid_get_health(task, lock_npc_hum) > 0 || __npc_hrp_to_lock_on_to == dungeon_boss_hrp)
                    {
            ...
            FOR_EACH_CHILD(enemies_folder, i, {
                ...
                if (enemy_hum)
                {
                    ...
                    if (dn)
                    {
                        ...
                        vm_deallocate(mach_task_self(), dn, dn_l);
                    }
                    __enemies[__i] = enemy;
        ...
        for (;;)
        {
            ...
            for (int i = 0 ; i < __enemy_count ; i++)
            {
                ...
                if (e_name)
                {
                    ...
                    if (!strstr(e_name, "'s Shadow"))
                    {
                        if (enemy_hum)
                        {
                            rbx_basepart_set_size(task, __enemy_hrps[i], new_hitbox_size);
                        }
                    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        for (;;)
        {
            if (boathacks_enabled && boatfly_enabled && rbx_instance_get_parent(task, rbx_instance_get_parent(task, my_humanoid_current_seat_part)) == boats_folder)
            {
                ...
                float min_dist = 200; //200 for frozen dimension, 2000 for prehistoric, 3000 for mirage
