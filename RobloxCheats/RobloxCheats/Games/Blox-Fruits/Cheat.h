...
#pragma mark CFrame writes
    ...
    BACKGROUND_PRIO_LOOP({
        if (locking_on_enemy && !bailing && !returning_from_rift && !(loop_goto_plr_enabled))
        {
...
#pragma mark - ESP -
    ...
    BASIC_ESP_ENGINE({
        ...
        if (mirage_island_exists)
        {
            ...
            ESP_Frame frame = {.x = shared_memory->WINDOW_H};
            frame.x = 0;
        ...
        if (kitsune_island_exists)
        {
            ...
            ESP_Frame frame;
            frame.x = 0;
            frame.y = shared_memory->WINDOW_H - 255;
            frame.w = shared_memory->WINDOW_W;
            frame.h = 40;
            shared_memory->ESP_BOX_HIDDEN_ARRAY[esp_index] = false;
            char text[MAX_ESP_TEXT_LENGTH];
        ...
        if (frozen_dimension_exists)
        {
            ...
            ESP_Frame frame;
            frame.x = 0;
            frame.y = shared_memory->WINDOW_H - 270;
            frame.w = shared_memory->WINDOW_W;
            frame.h = 40;
            shared_memory->ESP_BOX_HIDDEN_ARRAY[esp_index] = false;
            char text[MAX_ESP_TEXT_LENGTH];
        ...
        if (prehistoric_island_exists)
        {
            ...
            ESP_Frame frame;
            frame.x = 0;
            frame.y = shared_memory->WINDOW_H - 285;
            frame.w = shared_memory->WINDOW_W;
            frame.h = 40;
            shared_memory->ESP_BOX_HIDDEN_ARRAY[esp_index] = false;
            char text[MAX_ESP_TEXT_LENGTH];
...
#pragma mark - stdin Inputs -
    ...
    BACKGROUND_PRIO_LOOP({
        ...
        if (strncmp(str, "loopgoto ", strlen("loopgoto ")) == 0)
        {
            char name[2000];
            sscanf(str, "loopgoto %s", name);
        ...
        if (strcmp(str, "noloopgoto\xa") == 0)
        {
            loop_goto_plr_enabled = false;
            printf("DISABLED LOOPGOTO\n");
        }
