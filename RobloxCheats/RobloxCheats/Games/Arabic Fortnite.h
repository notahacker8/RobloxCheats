...
void arabic_fortnite_cheat(task_t task)
{
    ...
    static vm_address_t my_uppertorso = 0;
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
    {
        for (;;)
        {
            ...
            my_uppertorso = rbx_instance_find_first_child(task, my_char, "UpperTorso");
            long player_count = 0;
    ...
    for (;;)
    {
        ...
        rbx_cframe_t ut_cframe = rbx_basepart_get_cframe(task, my_uppertorso);
        vector3_t lv = rbx_get_cframe_look_vector(ut_cframe);
