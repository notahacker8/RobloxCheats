...
void dead_rails_cheat(task_t task)
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        FOR_EACH_CHILD(runtimeitems_folder, {
            char append = true;
            ...
            if (name)
            {
                vm_address_t bbg = rbx_instance_find_first_child_of_class(task, child, "BillboardGui");
                
                char text[1000];
                bzero(text, sizeof(text));
                
                FOR_EACH_CHILD(bbg, {
                    if (rbx_instance_is_a(task, child, "TextLabel"))
                    {
                        long str_l = 0;
