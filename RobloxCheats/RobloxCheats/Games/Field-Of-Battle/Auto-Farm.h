...
void field_of_battle_auto_farm(char sword_key)
{
    ...
    vm_offset_t input_usleep_time_offset = file_gdso(__LIBESP_DYLIB_PATH__, "INPUT_USLEEP_TIME");
    vm_offset_t input_queue_offset = file_gdso(__LIBESP_DYLIB_PATH__, "INPUT_QUEUE");
    vm_offset_t input_queue_count_offset = file_gdso(__LIBESP_DYLIB_PATH__, "INPUT_QUEUE_COUNT");
    vm_offset_t input_queue_finished_offset = file_gdso(__LIBESP_DYLIB_PATH__, "INPUT_QUEUE_FINISHED");
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^
    {
        for (;;)
        {
            ...
            {
                //send_left_click(task, load_address, input_queue_offset, input_queue_count_offset, input_queue_finished_offset, (NSPoint){.x = 200, .y = 200}, 10000); //Anti-Afk
            ...
            else
            {
                ...
                if (char_tool == 0)
                {
                    ...
                    //send_simple_keypress(task, load_address, input_queue_offset, input_queue_count_offset, input_queue_finished_offset, keycode_for_character(sword_key), 100000);
                }
