...
void field_of_battle_auto_farm(task_t task, char sword_key)
{
    ...
    shared_memory -> INPUT_QUEUE_COUNT = 2;
        ...
        for (;;)
        {
            ...
            else
            {
                ...
                if (char_tool == 0)
                {
                    ...
                    shared_memory->INPUT_QUEUE[1] = (Input){.type = INPUT_TYPE_KEY_DOWN, .should_execute = true, .keycode = keycode_for_character(sword_key)};
                    shared_memory->INPUT_QUEUE_FINISHED = false;
                    usleep(10000);
                    while (shared_memory->INPUT_QUEUE_FINISHED == false) {usleep(1000);}
                    shared_memory->INPUT_QUEUE[1] = (Input){.type = INPUT_TYPE_KEY_UP, .should_execute = true, .keycode = keycode_for_character(sword_key)};
                    shared_memory->INPUT_QUEUE_FINISHED = false;
                    while (shared_memory->INPUT_QUEUE_FINISHED == false) {usleep(10000);}
                }
