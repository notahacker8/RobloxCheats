...
void field_of_battle_end_round(task_t task)
{
    LIBESP_DYLIB_SETUP
    ...
    static vm_address_t general = 0;
    static vm_address_t general_hum = 0;
    static vm_address_t general_torso = 0;
    static vm_address_t general_torso_cframe_address = 0;
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        for (;;)
        {
            ...
            general = rbx_instance_find_first_child_partial(task, enemy_char_folder, "General");
            general_hum = rbx_instance_find_first_child_of_class(task, general, "Humanoid");
            general_torso = rbx_instance_find_first_child(task, general, "Torso");
            //rbx_print_descendants(task, general, 0, 1);
            sleep(1);
        }
    });
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
                   {
        rbx_cframe_t new_cf = DEFAULT_CFRAME;
        new_cf.pos.y = -99999;
        vector3_t new_sz = {.x = 0, .y = 0, .z = 0};
        for (;;)
        {
            if (general_torso)
            {
                rbx_humanoid_set_health(task, general_hum, 0);
                rbx_basepart_set_cframe(task, general_torso, new_cf);
            }
            else
            {
                sleep(1);
...
void hook_test(task_t task)
{
    ...
    vm_address_t addr = task_base_address + 0x3053038;
    
    char hook_code[] = "\xff\x43\x00\xd1\xe9\x03\x00\xa9\x60\x01\x00\x10\x00\x00\x40\xf9\x3f\x01\x00\xeb\xa0\x00\x00\x54\xe9\x03\x40\xa9\xff\x43\x00\x91\x1f\x20\x03\xd5\xc0\x03\x5f\xd6\xe9\x03\x40\xa9\xff\x43\x00\x91\xc0\x03\x5f\xd6\x00\x00\x00\x00\x00\x00\x00\x00";
    /*
     sub sp, sp, 16
        stp x9, x0, [sp]
        adr x0, blacklist
    ...
    vm_address_t a = 0x1223B6B48;
    memcpy(hook_code + hook_code_size - 8, &a, 8);
    
    //vm_address_t hook_mem = vm_allocate_rwx(task, hook_code, hook_code_size);
    vm_address_t hook_mem = task_base_address + 0x7f00000; //vm_allocate_rwx(task, hook_code, hook_code_size);
    vm_write(task, hook_mem, hook_code, hook_code_size);
    uint32_t og_inst = vm_read_uint32_t_value(task, addr);
    uint32_t jmpto_b = encode_b(addr, hook_mem);
    uint32_t jmpback_b1 = encode_b(hook_mem + hook_code_size - 24, addr + 4);
    uint32_t jmpback_b2 = encode_b(hook_mem + hook_code_size - 12, addr + 4);
    
    vm_write(task, hook_mem + hook_code_size - 24, (vm_address_t)&jmpback_b1, 4);
    vm_write(task, hook_mem + hook_code_size - 28, (vm_address_t)&og_inst, 4);
    vm_write(task, hook_mem + hook_code_size - 12, (vm_address_t)&jmpback_b2, 4);
