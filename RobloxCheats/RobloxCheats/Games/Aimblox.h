...
/*
void aimblox_cheat(task_t task)
{
    printf("- AIMBLOX -\n");
    ...
    for (;;)
    {
        ...
        if (player_list)
        {
            ...
            {
                vm_address_t plr = player_list[i].child_address;
                vm_address_t character = rbx_player_get_character(task, plr);
                vm_address_t ally_overhead = rbx_instance_find_first_child(task, character, "AllyOverhead");
                if (ally_overhead == 0)
                {
                    rbx_print_descendants(task, character, 0, 0);
                }
            }
...
#define RBX_AIMBLOX_MAX_PLAYER_COUNT 25
...
void aimblox_cheat(task_t task)
{
    ...
    vm_address_t local_player = rbx_instance_find_first_child_of_class(task, players_service, "Player");
    vm_address_t cam_cframe_value = rbx_instance_find_first_child(task, local_player, "CAM");
    
    static vm_address_t enemy_heads[RBX_AIMBLOX_MAX_PLAYER_COUNT];
            ...
            if (player_list)
            {
                for (int i = 0 ; i < player_count ; i++)
                {
                    ...
                    if (player && player != local_player)
                    {
                        ...
                        if (character)
                        {
                            vm_address_t allyoverhead = rbx_instance_find_first_child_of_class(task, character, "BillboardGui");
                            if (allyoverhead == 0)
                            {
