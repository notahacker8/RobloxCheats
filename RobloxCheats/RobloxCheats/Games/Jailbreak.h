...
void jailbreak_cheat(void)
{
    ...
    vm_offset_t keys_down_offset = file_gdso(__LIBESP_DYLIB_PATH__, "KEYS_DOWN");
    ...
    int esp_count = max_player_count * 2;
    ...
    static ESP_Color player_team_esp_colors[max_player_count];
    static vm_address_t player_torsos[max_player_count];
    static char player_names[max_player_count][MAX_ESP_TEXT_LENGTH];
...
#pragma mark - ESP -
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
    {
        for (;;)
        {
            long __current_player_count = current_player_count;
            if (__current_player_count > 0 && __current_player_count < (max_player_count + 1))
            {
                int var_index = 0;
                int esp_index = 0;
                
                for (long i = 0 ; i < __current_player_count ; i++)
                {
                    usleep(esput/__current_player_count);
