...
void generic_cheat(task_t task)
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        for (;;)
        {
            ...
            FOR_EACH_DESCENDANT(task, workspace, 10000000, 100, {
                ...
                if (rbx_instance_is_a(task, obj, "TextLabel") || rbx_instance_is_a(task, obj, "TextButton"))
                {
                    ...
                    {
                        //printf("%s\n", str);
                        //rbx_print_ancestors(task, obj, 9);
                ...
                if (!GetProcessPID(&psn, &_p))
                {
                    ...
                    else
                    {
                        ...
                        if (use_suspend)
                        {
                            char ps[200];
                            sprintf(ps, "%d", pid);
                            char *argv[] = {__CPUTHROTTLE_PATH__, ps, "10", NULL};
                            
                            if (ctpid == -1)
                            {
                                //printf("starting cputhrottle\n");
                                int err = posix_spawn(&ctpid, __CPUTHROTTLE_PATH__, NULL, NULL, argv, NULL);
                                if (err)
                                {
                                    ctpid = -1;
