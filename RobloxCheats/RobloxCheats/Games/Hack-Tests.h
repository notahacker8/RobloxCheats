...
void find_object_offsets(task_t task)
{
    ...
    
    {
        ...
        for (long i = 0 ; i < 0x200/8 ; i++)
        {
            vm_address_t a = ret_addr - (i*8);
            if (vm_read_uint64_value(task, a) == a - 8)
            {
                intvalue = a-8;
                i = 0x200/8;
            }
        ...
        RBX_WHAT_GAME_POINTS_TO_OFFSET = (int)(vm_read_uint64_value(task, game) - task_base_address);
    }
    ...
    *(vector3_t*)known_bytes = (vector3_t){.x = 8, .y = 1, .z = 2};
    ...
    shared_memory->FUNCTION_QUEUE[2].address = task_base_address + 0x15bbc; //0x142B8;
