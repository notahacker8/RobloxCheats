...
void find_object_offsets(task_t task)
{
    ...
    vm_address_t game = rbx_find_game_address(task); //0x127984B48;
    printf("\n-- GAME CHECK --\n");
    if (!game)
    {
    ...
    *(int*)known_bytes = 42069;
    ...
    vm_offset_t stringvalue_value_offset = intvalue_value_offset + 8;
    vm_offset_t cframevalue_value_offset = intvalue_value_offset + 8;
    
    bzero(known_bytes, sizeof(known_bytes));
    *(rbx_cframe_t*)known_bytes = vm_read_rbx_cframe_value(task, ofo_camera_cframevalue + cframevalue_value_offset);
    ...
    *(vm_address_t*)known_bytes = my_character;
    ...
    bzero(known_bytes, sizeof(known_bytes));
    *(vm_address_t*)known_bytes = vm_read_uint64_value(task, ofo_player_team_ov + intvalue_value_offset);
    ...
    if (kr == KERN_SUCCESS)
    {
        for (long i = 0 ; i < (size - 8)/8; i++)
        {
            ...
            if (is_valid_pointer(task, a) && rbx_instance_get_self(task, a) != a)
            {
                ...
                rbx_cframe_t known_cframe1 = vm_read_rbx_cframe_value(task, ofo_part_cframe1_cv + cframevalue_value_offset);
                ...
                {
                    rbx_cframe_t known_cframe2 = vm_read_rbx_cframe_value(task, ofo_part_cframe2_cv + cframevalue_value_offset);
                    vm_address_t pp2 = vm_read_uint64_value(task, ofo_part2 + (i * 8));
    ...
    *(vector3_t*)known_bytes = (vector3_t){.x = 8, .y = 1, .z = 2};
    vm_offset_t basepart_properties_size_offset = scan_bytes_offset(task, vm_read_uint64_value(task, ofo_part1 + basepart_properties_offset), size, known_bytes, 4, 4);
    ...
    if (kr == KERN_SUCCESS)
    {
        for (long i = 0 ; i < size; i++)
        {
            ...
            if (a == true)
            {
                if (vm_read_uint8_value(task, ofo_frame2 + i) == false)
                {
                    if (vm_read_uint8_value(task, ofo_textlabel1 + i) == true)
                    {
                        if (vm_read_uint8_value(task, ofo_textlabel2 + i) == true)
                        {
    ...
    
    {
        ...
        vm_address_t zap_pp = vm_read_uint64_value(task, zap + basepart_properties_offset);
        vm_address_t xap_pp = vm_read_uint64_value(task, xap + basepart_properties_offset);
        vm_address_t wpp_pp = vm_read_uint64_value(task, wpp + basepart_properties_offset);
        ...
        if (kr == KERN_SUCCESS)
        {
            for (long i = 0 ; i < size; i++)
            {
                ...
                if (a == true)
                {
                    if (vm_read_uint8_value(task, zap_pp + i) == false)
                    {
                        if (vm_read_uint8_value(task, wpp_pp + i) == true)
                        {
    ...
    for (;;)
    {
        ...
        printf("what game points to %p\n", vm_read_uint64_value(task, game));
        sleep(1);
    }
    ...
    shared_memory->FUNCTION_QUEUE[2].address = task_base_address + 12258; //0x142B8;
    shared_memory->FUNCTION_QUEUE[2].type = 4;
    shared_memory->FUNCTION_QUEUE[2].async_type = FUNCTION_ASYNC_TYPE_MAIN;
    
    char args[48];
    memcpy(args, &t, 8);
    memcpy(args + 8, &workspace, 8);
    memcpy(args + 16, &t, 8);
    long v = 1;
    memcpy(args + 24, &v, 8);
