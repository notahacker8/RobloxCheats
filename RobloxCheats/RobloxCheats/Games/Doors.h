...
#pragma mark - Object Loop -
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
    {
        for (;;)
        {
            ...
            long workspace_cc;
            rbx_child_t* workspace_children = rbx_instance_get_children(task, workspace, &workspace_cc);
            if (workspace_children)
            {
                for (long i = 0 ; i < workspace_cc ; i++)
                {
                    vm_address_t child = workspace_children[i].child_address;
                    long cnl = 0;
                    char* cn = rbx_instance_get_name(task, child, &cnl);
                    if (cn)
                    {
                        if (strcmp(cn, "RushMoving") == 0)
                        {
                            vm_address_t monster_part = rbx_instance_find_first_child_of_class(task, child, "Part");
                ...
                vm_deallocate(mach_task_self_, (vm_address_t)workspace, workspace_cc * sizeof(rbx_child_t));
            }
                    ...
                    if (current_rooms_count > 1)
                    {
                        ...
                        if (current_room)
                        {
                            ...
                            if (!seekmoving)
                            {
                                ...
                                //rbx_print_descendants(task, current_room_assets_folder, 0, 2);
                                        ...
                                        if (asset_children)
                                        {
                                            for (int _i = 0 ; _i < asset_child_count ; _i++)
                                            {
                                                ...
                                                if (name)
                                                {
                                                    ...
                                                    if (container)
                                                    {
                                                        //rbx_print_descendants(task, container, 0, 1);
...
#pragma mark - ESP -
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
    {
        ...
        {
            int esp_index = 0;
            long __esp_object_count = esp_object_count;
            if (__esp_object_count > 0 && __esp_object_count < MAX_ESP_COUNT)
            {
                for (int i = 0 ; i < __esp_object_count ; i++)
                {
                    ...
                    usleep(esput/__esp_object_count);
                }
            }
            else
            {
