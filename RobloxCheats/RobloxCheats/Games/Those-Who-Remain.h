...
#pragma mark - RPM Search -
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        for (;;)
        {
            ...
            {
                if (shared_memory->KEYS_DOWN_ONCE['l'])
                {
                    if (did_save_cframe)
                    {
                        rbx_basepart_set_cframe(task, my_hrp, saved_cframe);
                    }
                    shared_memory->KEYS_DOWN_ONCE['l'] = false;
                }
                usleep(1000);
            }
            else
...
#pragma mark - stdin Inputs -
    ...
    STDIN_INPUT_ENGINE({
                                  ...
                                  bot_mode = false; esp_wait = 50; noclip_enabled = false; esp_enabled = true;)
        {
            ...
            INSERT_STDIN_MULTIARG_INPUT("pres %lf", {printf("SET AMMO[0] (PRIMARY) RESERVE TO %lf\n", TWR_data->ammo_sets[0]);}, 1, &TWR_data->ammo_sets[0]);
            INSERT_STDIN_MULTIARG_INPUT("sres %lf", {printf("SET AMMO[1] (SECONDARY) RESERVE TO %lf\n", TWR_data->ammo_sets[1]);}, 1, &TWR_data->ammo_sets[1]);
            INSERT_STDIN_MULTIARG_INPUT("pmag %lf", {printf("SET AMMO[0] (PRIMARY) MAG TO %lf\n", TWR_data->mag_sets[0]);}, 1, &TWR_data->mag_sets[0]);
            INSERT_STDIN_MULTIARG_INPUT("smag %lf", {printf("SET AMMO[1] (SECONDARY) MAG TO %lf\n", TWR_data->mag_sets[1]);}, 1, &TWR_data->mag_sets[1]);
