...
void arsenal_cheat(void)
{
    ...
    vm_offset_t left_mouse_down_offset = file_gdso(__LIBESP_DYLIB_PATH__, "LEFT_MOUSE_DOWN");
...
#pragma mark - ESP -
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
    {
        for (;;)
        {
            long __enemy_count = enemy_count;
            if (esp_enabled &&
                __enemy_count > 0 &&
                __enemy_count < (max_player_count + 1))
            {
                int esp_index = 0;
