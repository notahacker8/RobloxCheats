...
void arsenal_cheat(task_t task)
{
    ...
    shared_memory->ESP_COUNT = 32; //We only draw boxes on players.
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        for (;;)
        {
            ...
            enemy_count = enemy_index;
            //bzero(enemy_torsos + (8 * enemy_count), (sizeof(enemy_torsos) - (8 * enemy_count)));
            usleep(200000);
        }
    });
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
    {
        for (;;)
        {
            ...
            long __current_enemy_count = enemy_count;
            if (esp_enabled && __current_enemy_count > 0)
            {
                int esp_index = 0;
                ...
                if (closest_enemy_torso)
                {
                    if (shared_memory->LEFT_MOUSE_DOWN == true)
                    {
                        ...
                        if (max_dist > dist && max_delta_dist > delta_dist)
                        {
                            if (aimbot_enabled)
                            {
                                vector3_t new_lv = vector3_unit(torso_cframe.pos, camera_cframe.pos);
