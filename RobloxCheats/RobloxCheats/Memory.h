...
unsigned long vm_read_8byte_value(task_t task, vm_address_t address)
{
    ...
    if (kr == KERN_SUCCESS)
    {
        __data = *(unsigned long*)read_data;
...
unsigned int vm_read_4byte_value(task_t task, vm_address_t address)
{
    ...
    if (kr == KERN_SUCCESS)
    {
        __data = *(unsigned int*)read_data;
        vm_deallocate(mach_task_self_, (vm_address_t)read_data, 4);
    }
    return __data;
}

//Outdated (probably, I haven't checked)
...
unsigned char vm_read_1byte_value(task_t task, vm_address_t address)
{
    ...
    if (kr == KERN_SUCCESS)
    {
        __data = *(unsigned char*)read_data;
...
VM_DEALLOCABLE char* vm_read_rbx_string(task_t task, vm_address_t address, long* inout_length)
{
    ...
    unsigned char short_length = vm_read_1byte_value(task, address + 0x17);
    //printf("%d\n", short_length);
    if (short_length < 23)
    {
        str_ptr = address;
    ...
    {
        str_ptr = vm_read_8byte_value(task, address);
        //printf("%p\n", str_ptr);
