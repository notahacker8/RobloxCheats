...
#define VM_READ_FUNCTION_CREATE(fn_name, data_type)\
data_type fn_name(task_t task, vm_address_t address)\
{\
    data_type data;\
...
VM_READ_FUNCTION_CREATE(vm_read_uint64_value, uint64_t);
VM_READ_FUNCTION_CREATE(vm_read_uint32_value, uint32_t);
VM_READ_FUNCTION_CREATE(vm_read_uint16_value, uint16_t);
VM_READ_FUNCTION_CREATE(vm_read_uint8_value, uint8_t);
VM_READ_FUNCTION_CREATE(vm_read_double_value, double);
VM_READ_FUNCTION_CREATE(vm_read_float_value, float);
VM_READ_FUNCTION_CREATE(vm_read_vector3_value, vector3_t);
VM_READ_FUNCTION_CREATE(vm_read_rbx_cframe_value, rbx_cframe_t);
...
{
    long x8 = vm_read_uint64_value(task, address);
    int w9 = vm_read_uint32_value(task, x8);
...
VM_DEALLOCABLE char* vm_read_rbx_string(task_t task, vm_address_t address, long* inout_len)
{
    mach_msg_type_number_t data_cnt;
    char* text = NULL;
    unsigned char short_length_x2 = vm_read_uint8_value(task, address);
    if (short_length_x2 == vm_read_uint64_value(task, address))
    {
        long long_length = vm_read_uint64_value(task, address + 8);
        vm_address_t str_ptr = vm_read_uint64_value(task, address + 16);
...
VM_DEALLOCABLE char* vm_read_rbx_string(task_t task, vm_address_t address, long* inout_length)
{
    ...
    unsigned char short_length = vm_read_uint8_value(task, address + 0x17);
    //printf("%d\n", short_length);
    if (short_length < 23)
    {
        str_ptr = address;
    ...
    {
        str_ptr = vm_read_uint64_value(task, address);
        //printf("%p\n", str_ptr);
        if (is_valid_pointer(task, str_ptr) == true)
        {
            length = vm_read_uint64_value(task, address + 8);
        }
        else
        {
            length = 0;
