...
{
    return vm_read_8byte_value(task, camera + RBX_CAMERA_CAMERASUBJECT_OFFSET);
}

void rbx_camera_set_camera_subject(task_t task, vm_address_t camera, vm_address_t new_subject)
{
    vm_write(task, camera + RBX_CAMERA_CAMERASUBJECT_OFFSET, (vm_offset_t)&new_subject, sizeof(void*));
}

rbx_cframe_t rbx_camera_get_cframe(task_t task, vm_address_t camera)
{
    return vm_read_rbx_cframe_value(task, camera + RBX_CAMERA_CFRAME_OFFSET);
}

void rbx_camera_set_cframe(task_t task, vm_address_t camera, rbx_cframe_t cf)
{
    vm_write(task, camera + RBX_CAMERA_CFRAME_OFFSET, (vm_address_t)&cf, sizeof(rbx_cframe_t));
    //vm_write(task, camera + RBX_CAMERA_CFRAME_OFFSET + 0x60, (vm_address_t)&cf, sizeof(rbx_cframe_t));
    return;
}
...
rbx_cframe_t rbx_camera_get_focus(task_t task, vm_address_t camera)
{
    ...
    return vm_read_rbx_cframe_value(task, camera + RBX_CAMERA_CFRAME_OFFSET + sizeof(rbx_cframe_t));
}

void rbx_camera_set_focus(task_t task, vm_address_t camera, rbx_cframe_t cf)
{
    vm_write(task, camera + RBX_CAMERA_CFRAME_OFFSET + sizeof(rbx_cframe_t), (vm_address_t)&cf, sizeof(rbx_cframe_t));
    //vm_write(task, camera + RBX_CAMERA_CFRAME_OFFSET + 0x60, (vm_address_t)&cf, sizeof(rbx_cframe_t));
    return;
}

void rbx_camera_set_focus_pos(task_t task, vm_address_t camera, vector3_t pos)
{
    vm_write(task, camera + RBX_CAMERA_CFRAME_OFFSET + sizeof(rbx_cframe_t) + 0x24, (vm_address_t)&pos, sizeof(vector3_t));
    //vm_write(task, camera + RBX_CAMERA_CFRAME_OFFSET + 0x60, (vm_address_t)&cf, sizeof(rbx_cframe_t));
    return;
}
...
{
    int a = (int)vm_read_4byte_value(task, camera + RBX_CAMERA_FIELDOFVIEW_OFFSET);
    float b = *((float*)&a);
    return b * (180/PI);
}
