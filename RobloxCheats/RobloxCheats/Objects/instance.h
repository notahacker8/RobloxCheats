        ...
        for (long i = 0 ; i < child_count ; i++)
        {
            ...
            if (cname != NULL)
            {
                ...
                if (strcmp(cname, class_name) == 0)
                {
                    ...
                    {
                        found_child = child;
                        i = 0;   //break the loop, without skipping the deallocating part;
                    }
                    vm_deallocate(mach_task_self_, (vm_address_t)_name, _name_len);
                }
                vm_deallocate(mach_task_self_, (vm_address_t)cname, cname_len);
...
#define FOR_EACH_DESCENDANT(task, instance, max_desc_count, child_depth, code)\
{\
...
{
    vm_address_t desc = 0;
    FOR_EACH_DESCENDANT(task, instance, max_d_count, depth, {
