...
{
    return vm_read_uint64_value(task, instance + RBX_INSTANCE_SELF_OFFSET);
}

vm_address_t rbx_instance_get_parent(task_t task, vm_address_t instance)
{
    return vm_read_uint64_value(task, instance + RBX_INSTANCE_PARENT_OFFSET);
}


char* rbx_instance_get_name(task_t task, vm_address_t instance, long* inout_len)
{
    vm_address_t name_address = vm_read_uint64_value(task, instance + RBX_INSTANCE_NAME_OFFSET);
...
{
    char* str = NULL;
    vm_address_t ci_ptr = vm_read_uint64_value(task, instance + RBX_INSTANCE_CLASS_INFO_OFFSET);
    if (ci_ptr)
    {
        vm_address_t class_name_addr = vm_read_uint64_value(task, ci_ptr + 8);
...
rbx_child_t* rbx_instance_get_children(task_t task, vm_address_t instance, long* inout_child_count)
{
    ...
    vm_address_t children_info_ptr = vm_read_uint64_value(task, instance + RBX_INSTANCE_CHILDREN_OFFSET);
    if (children_info_ptr)
    {
        vm_address_t child_list_start = vm_read_uint64_value(task, children_info_ptr);
        vm_address_t child_list_end = vm_read_uint64_value(task, children_info_ptr + 8);
...
{
    return (rbx_instance_get_parent(task, instance) && rbx_instance_get_self(task, instance) == instance);
}
...
void rbx_get_descendants(task_t task,
                         ...
                         int max_d_count)
{
    if (instance == 0 || current_recurse_count > max_recurse_count || ((*current_d_count) - 1) > max_d_count)
    {
        return;
    }
    long cc = 0;
