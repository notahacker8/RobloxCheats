...
{
    return vm_read_uint64_t_value(task, intvalue + RBX_INTVALUE_VALUE_OFFSET);
}
...
{
    long a = vm_read_uint64_t_value(task, numbervalue + RBX_INTVALUE_VALUE_OFFSET);
    return *((double*)(&a));
}
...
{
    return vm_read_uint64_t_value(task, value + RBX_INTVALUE_VALUE_OFFSET);
}
...
{
    return vm_read_uint8_t_value(task, boolvalue + RBX_INTVALUE_VALUE_OFFSET);
}

void rbx_boolvalue_set_value(task_t task, vm_address_t boolvalue, char value)
{
    vm_write(task, boolvalue + RBX_INTVALUE_VALUE_OFFSET, (vm_offset_t)&value, 1);
}

char* rbx_stringvalue_get_value(task_t task, vm_address_t stringvalue, long* inout_length)
{
    return vm_read_rbx_string(task, stringvalue + RBX_STRINGVALUE_VALUE_OFFSET, inout_length);
}
