...
FREEABLE char* read_file(char* path, long* _Nullable inout_length)
{
    char* bytes = NULL;
    struct stat buf;
    if (stat(path, &buf) == 0)
...
#include "Games/Blox-Fruits/Chest-Collect.h"
#include "Games/Blox-Fruits/NPC-Finder.h"
#include "Games/Blox-Fruits/Auto-Hunt.h"
...
int main(int argc, char** argv)
{
    ...
    bzero(__CPUTHROTTLE_PATH__, PATH_MAX);
    bzero(__LIBESP_DYLIB_PATH__, PATH_MAX);
    bzero(__OFFSETS_TXT_PATH__, PATH_MAX);
    bzero(__COOKIE_TXT_PATH__, PATH_MAX);
    ...
    if (argc > 1)
    {
        ...
        if (strcmp(argv[1], "launch_game") == 0)
        {
            if (argc > 2)
            {
                if (argc > 3)
                {
                    char* placeid = argv[2];
                    char* serverid = argv[3];
                    if (strcmp(serverid, "NULL") == 0)
                    {
                        serverid = NULL;
                    }
                    rbx_launch_game(__COOKIE_TXT_PATH__, placeid, serverid);
                }
                else
                {
                    fprintf(stderr, "error: lacking argument(s): '%s'\n", "serverid");
                    exit(0);
                }
            }
            else
        ...
        else if (strcmp(argv[1], "gameinfo") == 0)
        {
            rbx_print_game_info(task);
            exit(0);
        }
        else if (strcmp(argv[1], "blox_fruits") == 0)
        {
            rbx_run_robloxcheats_self_termination_thread();
            blox_fruits_cheat(task);
        }
        else if (strcmp(argv[1], "blox_fruits_chest_farm") == 0)
        {
            rbx_run_robloxcheats_self_termination_thread();
            blox_fruits_chest_farm(task);
        }
        else if (strcmp(argv[1], "blox_fruits_auto_hunt") == 0)
        {
            rbx_run_robloxcheats_self_termination_thread();
            blox_fruits_auto_hunt(task);
        }
        else if (strcmp(argv[1], "phantomforces") == 0)
        {
            rbx_run_robloxcheats_self_termination_thread();
            phantom_forces_cheat(task);
        }
        else if (strcmp(argv[1], "doors") == 0)
        {
            rbx_run_robloxcheats_self_termination_thread();
            doors_cheat(task);
        }
        else if (strcmp(argv[1], "cpu_limit") == 0)
        {
            if (argc > 2)
            {
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
                {
                    for (;;)
                    {
                        ...
                        if (pids_by_name("cputhrottle_remake", pa) == 0)
                        {
                            char cmd[1000];
                            int p = -1;
                            if(pid_for_task(task, &p) == KERN_SUCCESS)
                            {
                                sprintf(cmd, "%s %d %f", __CPUTHROTTLE_PATH__, p, atof(argv[2]));
                                system(cmd);
                            }
                        }
                        sleep(3);
        ...
        else if (strcmp(argv[1], "arsenal") == 0)
        {
            rbx_run_robloxcheats_self_termination_thread();
            arsenal_cheat(task);
        }
        else if (strcmp(argv[1], "field_of_battle_legendary_gem_collect") == 0)
        {
            rbx_run_robloxcheats_self_termination_thread();
            field_of_battle_collect_legendary_gem(task);
        }
        else if (strcmp(argv[1], "field_of_battle_auto_farm") == 0)
        {
            if (argc > 2)
            {
                rbx_run_robloxcheats_self_termination_thread();
                field_of_battle_auto_farm(task, *argv[2]);
            }
            else
            {
                fprintf(stderr, "error: lacking argument(s): '%s'\n", "sword_key");
        ...
        else if (strcmp(argv[1], "jailbreak") == 0)
        {
            rbx_run_robloxcheats_self_termination_thread();
            jailbreak_cheat(task);
        }
        else if (strcmp(argv[1], "find_offsets") == 0)
        {
            find_object_offsets(task);
            find_function_offsets(task);
            printf("Finished finding offsets\n");
            exit(0);
        }
        else
        {
            printf("Couldn't find command for '%s'\n", argv[1]);
            exit(0);
        }
    }
    else
    {
        ...
        printf("\tcpu_limit [(float) percentage]\n");
        printf("\tlaunch_game [(char*) placeid] [(char* _Nullable) serverid]\n");
        printf("\tgameinfo\n");
        printf("\tfind_offsets\n");
        printf("\tblox_fruits\n");
        printf("\tblox_fruits_chest_farm\n");
        printf("\tblox_fruits_auto_hunt\n");
        printf("\tdoors\n");
        printf("\tphantomforces\n");
        printf("\tfield_of_battle_auto_farm [(char) sword_key]\n");
        printf("\tfield_of_battle_legendary_gem_collect\n");
        printf("\tjailbreak\n");
        printf("\tarsenal\n");
        printf("\nexample: ./RobloxCheats launch_game 7449423635 120c51ee-b06f-49ea-bc59-ccad1dfa86d4\n\n");
#ifndef TESTING
        exit(0);
#endif
    }
    //Test stuff here
#ifdef TESTING
    //printf("%s\n", rbx_datamodel_get_jobid(task, rbx_find_game_address(task)));
    //printf("%llu\n", rbx_get_placeid(task));
    //rbx_print_descendants(task, rbx_find_game_address(task), 0, 2);
    //rbx_print_game_info(task);
    //rbx_launch_game(__COOKIE_TXT_PATH__, "7449423635", "120c51ee-b06f-49ea-bc59-ccad1dfa86d4");
    //blox_fruits_cheat(task);
    //blox_fruits_chest_farm(task);
    //blox_fruits_find_master_of_auras(task);
