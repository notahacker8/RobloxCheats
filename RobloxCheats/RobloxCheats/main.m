...
void rbx_smooth_fly_patch(task_t task, vm_address_t task_base_address, char status)
{
    ...
    char orig_bytes[4] = {"\x01\x81\x05\xAD"};
...
void printAllSharedCookies(void) {
    NSHTTPCookieStorage *storage = [NSHTTPCookieStorage sharedHTTPCookieStorage];
    NSArray<NSHTTPCookie *> *cookies = [storage cookies];

    if (cookies.count == 0) {
        NSLog(@"No cookies in shared storage.");
        return;
    }

    NSLog(@"Shared cookies:");
    for (NSHTTPCookie *cookie in cookies) {
        ...
        NSLog(@"Secure: %@", cookie.isSecure ? @"YES" : @"NO");
        NSLog(@"HTTPOnly: %@", cookie.isHTTPOnly ? @"YES" : @"NO");
    }
...
int main(int argc, char** argv)
{
   ...
   //pid_t a[4096]; int c = pids_by_name("RobloxPlayer", a); char p[1000]; sprintf(p, "/Users/dimitriarmendariz/Documents/RobloxCookies/%d.txt", c); rbx_launch_game(p, RBX_TDS_LOBBY_PLACE_ID_STR, NULL);
    //rbx_launch_game("/Users/dimitriarmendariz/Documents/RobloxCookies/0.txt\0", RBX_TDS_LOBBY_PLACE_ID_STR, NULL);
    //rbx_launch_game(__COOKIE_TXT_PATH__, "15816797935", NULL);
    //bg_prio_auto(task, pid, false);
    ...
    //tds_play_round(task, RBX_TDS_MODE_TYPE_MOLTEN_WITH_SCOUT_DEATH_FARM, molten_with_scouts_stained_temple_placements, true, false, true, false, NULL, 0, true);
    //tds_collect(task);
