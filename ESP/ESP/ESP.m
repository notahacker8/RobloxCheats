...
void initialize(void)
{
            ...
            if (SHARED_MEMORY->ESP_ALLOCATED == false && SHARED_MEMORY->ESP_ENABLED == true)
            {
                dispatch_async(dispatch_get_main_queue(), ^
                {
                    for (int i = 0 ; i < MAX_ESP_COUNT ; i++)
                    {
                        ...
                        esp_box.bezeled = NO;
                        [esp_box setFont: [NSFont fontWithName:@"Menlo Bold" size:15]];
                ...
                dispatch_async(dispatch_get_main_queue(), ^
                {
                    ...
                    for (int i = 0 ; i < c ; i++)
                    {
                        if (i < MAX_ESP_COUNT)
                        {
                            ...
                            NSAttributedString *nsas = [[NSAttributedString alloc]
                                                        initWithString:objc_str
                                                        attributes:
                                                            @{
                            ...
                            esp_box.attributedStringValue = nsas;
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
    {
        for (;;)
        {
            ...
            if (NSApp.isActive)
            {
                int c = SHARED_MEMORY->INPUT_COUNT;
                for (int i = 0 ; i < c ; i++)
                {
                    if (i < MAX_INPUT_COUNT)
                    {
                        ...
                        if (input.should_execute == true)
                        {
                            if (input.type == INPUT_TYPE_MOUSE_MOVE)
                            {
                                ...
                                [NSApp.mainWindow sendEvent:[NSEvent mouseEventWithType:NSEventTypeMouseMoved location:NSPointFromCGPoint(point) modifierFlags:0 timestamp:0 windowNumber:0 context:NULL eventNumber:0 clickCount:0 pressure:0]];
                            }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
    {
        for (;;)
        {
            ...
            int c = SHARED_MEMORY->FUNCTION_COUNT;
            for (int i = 0 ; i < c ; i++)
            {
