...
void SIGTRAP_HANDLER(int signum, siginfo_t* signal_info, void* context)
{
    ...
    //libESPSharedMemoryObject __SHARED_MEMORY = *SHARED_MEMORY;
...
{
   {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
        ...
        {
            libESPSharedMemoryObject __SHARED_MEMORY = *SHARED_MEMORY;
            if (__SHARED_MEMORY.ESP_ALLOCATED == true && __SHARED_MEMORY.ESP_ENABLED == true)
            {
                dispatch_async(dispatch_get_main_queue(), ^
                               {
                    for (int i = 0 ; i < MAX_ESP_COUNT ; i++)
            ...
            sleep(__SHARED_MEMORY.REFRESH_SLEEP_TIME);
        }
    });
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
    {
        for (;;)
        {
            ...
            if (__SHARED_MEMORY.ESP_ALLOCATED == true && __SHARED_MEMORY.ESP_ENABLED == true)
            {
                dispatch_async(dispatch_get_main_queue(), ^
                {
                    for (int i = 0 ; i < __SHARED_MEMORY.ESP_COUNT ; i++)
                    {
                        if (i < MAX_ESP_COUNT)
                        {
                            ESPTextField* esp_box = (__bridge ESPTextField*)(__SHARED_MEMORY.ESP_BOX_ARRAY[i]);
                            
                            ESP_Color espbc = __SHARED_MEMORY.ESP_BOX_COLOR_ARRAY[i];
                            char* c_str = (char*)((vm_address_t)__SHARED_MEMORY.ESP_BOX_TEXT_ARRAY + (i * MAX_ESP_TEXT_LENGTH));
                            ...
                            NSRect espbf = __SHARED_MEMORY.ESP_BOX_FRAME_ARRAY[i];
                            esp_box.frame = espbf;
                            esp_box.hidden = __SHARED_MEMORY.ESP_BOX_HIDDEN_ARRAY[i];
                            esp_box.layer.borderWidth = __SHARED_MEMORY.ESP_BOX_BORDER_WIDTH_ARRAY[i];
            ...
            usleep(__SHARED_MEMORY.ESP_USLEEP_TIME);
        }
    });
            ...
            if (__SHARED_MEMORY.ESP_ALLOCATED == true && __SHARED_MEMORY.ESP_ENABLED == true)
            {
                dispatch_async(dispatch_get_main_queue(), ^
                {
                    for (int i = 0 ; i < __SHARED_MEMORY.ESP_COUNT ; i++)
                    {
                        ...
                        {
                            ESPTextField* esp_box = (__bridge ESPTextField*)(__SHARED_MEMORY.ESP_BOX_ARRAY[i]);
                            [esp_box setFont: [NSFont fontWithName:[NSString stringWithUTF8String:__SHARED_MEMORY.ESP_TEXT_FONT_NAME] size:__SHARED_MEMORY.ESP_TEXT_FONT_SIZE]];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^
    {
        for (;;)
        {
            ...
            usleep(__SHARED_MEMORY.INPUT_USLEEP_TIME);
            //printf("Cjecking if App is active\n");
            if (NSApp.isActive)
            {
                //printf("App is active\n");
                for (int i = 0 ; i < __SHARED_MEMORY.INPUT_COUNT ; i++)
                {
                    if (i < MAX_INPUT_COUNT)
                    {
                        Input input = __SHARED_MEMORY.INPUT_QUEUE[i];
